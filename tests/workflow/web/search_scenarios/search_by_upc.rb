...
query="select count(*) from skus where upc=#{$test_data['upc']}"
...
upcStyleCount=print_rows['count(*)']
 end
 ...
 #    end
    ...
    #   puts "Styles count for the style name from DB: " + nameStylesCount.to_s
      puts "Styles count for the style number from DB: " + upcStyleCount.to_s
    # puts "Styles count for the event from DB: " + eventIdStylesCount.to_s
    #  search by brand
    Searchstylespageobject.search("UPC",$test_data['upc'].to_s)
    Grid.gridroot
    search_results_row=Grid.getRows_all
    puts search_results_row.size
  assert_equal(search_results_row.size, upcStyleCount, "The correct style count must be returned")
  ...
  assert_equal(edit_buttons.size, upcStyleCount, "Edit Style buttons are not displayed as expected.")
  ...
  assert_equal(events_buttons.size, upcStyleCount, "View Events buttons are not displayed as expected.")
...
assert_equal(skus_buttons.size, upcStyleCount, "View Events buttons are not displayed as expected.")
...
assert_equal(persistent_buttons.size, upcStyleCount, "View Events buttons are not displayed as expected.")
