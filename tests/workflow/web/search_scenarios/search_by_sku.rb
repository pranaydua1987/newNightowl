...
query="select count(*) from skus where sku=#{$test_data['sku']}"
...
skuStyleCount=print_rows['count(*)']
 end
 ...
 #    end
    ...
    #   puts "Styles count for the style name from DB: " + nameStylesCount.to_s
      puts "Styles count for the style number from DB: " + skuStyleCount.to_s
    # puts "Styles count for the event from DB: " + eventIdStylesCount.to_s
    #  search by brand
    Searchstylespageobject.search("Sku",$test_data['sku'].to_s)
    Grid.gridroot
    search_results_row=Grid.getRows_all
    puts search_results_row.size
  assert_equal(search_results_row.size, skuStyleCount, "The correct style count must be returned")
  ...
  assert_equal(edit_buttons.size, skuStyleCount, "Edit Style buttons are not displayed as expected.")
  ...
  assert_equal(events_buttons.size, skuStyleCount, "View Events buttons are not displayed as expected.")
...
assert_equal(skus_buttons.size, skuStyleCount, "View Events buttons are not displayed as expected.")
...
assert_equal(persistent_buttons.size, skuStyleCount, "View Events buttons are not displayed as expected.")
