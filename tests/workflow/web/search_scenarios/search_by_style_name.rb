...
# #   # puts "We"
  ...
  puts print_rows['count(*)'] - 1
styleNameCount=print_rows['count(*)'] - 1
 end
 # query="select count(*) from styles where name=\"#{$test_data['style_name']}\""
 # result=Comman.query_results(query)
 #
 ...
 #    end
    ...
    #   puts "Styles count for the style name from DB: " + nameStylesCount.to_s
      puts "Styles count for the style number from DB: " + styleNameCount.to_s
  ...
  assert_equal(search_results_row.size, styleNameCount, "The correct style count must be returned")
  ...
  assert_equal(edit_buttons.size, styleNameCount, "Edit Style buttons are not displayed as expected.")
  ...
  assert_equal(events_buttons.size, styleNameCount, "View Events buttons are not displayed as expected.")
...
assert_equal(skus_buttons.size, styleNameCount, "View Events buttons are not displayed as expected.")
...
assert_equal(persistent_buttons.size, styleNameCount, "View Events buttons are not displayed as expected.")
